...
async def show_event_menu(update: Update, _: CallbackContext):
    ...
    message_text = "Мы организуем показы спектаклей, проводим открытые репетиции, мастер-классы, участвуем в благотворительных ярмарках и фестивалях."
...
к нашей организации."""
    ...
    if len(events) == 0:
        await _process_no_events(
            query=query,
            closing_message=(
                "Оставайтесь на связи, чтобы первыми узнавать о наших будущих мероприятиях. "
    ...
    await _send_event_messages(
        query=query, events=events, message_template=EVENT_MESSAGE_TEMPLATE
    )
    await _send_closing_message(
        query=query,
        closing_message=(
            "Вы можете подписаться на уведомления об анонсах, чтобы первыми узнавать о наших"
...
async def other_help_subscribe_to_notifications(update: Update, _: CallbackContext):
    ...
    await query.answer()
    message_text = (
        """Спасибо за отклик! Вы будете получать уведомления о наших потребностях."""
    )
    user_id = query.from_user.id
    response = await async_send_json_post_request(
        url=settings.NOTIFICATIONS_API_URL, data={"user_id": user_id}
    )
    message_text = await _check_api_response_status(
        message_text=message_text, response=response, user_id=user_id
    )
...
async def notify_subscribers_about_new_event(event_data: Dict, bot: Bot) -> None:
    ...
    for user_data in subscribers:
        ...
        await _send_message_or_handle_error(
            bot=bot, message_text=message, user_id=user_data["user_id"]
        )
...
async def _send_message_or_handle_error(bot, message_text, user_id):
    try:
        await bot.send_message(
            chat_id=user_id, text=message_text, parse_mode=ParseMode.HTML
        )
...
async def _handle_bot_block_error(error: TelegramError, user_id: int) -> None:
    ...
    if "Forbidden: bot was blocked by the user" in str(error):
        logger.info(
            f"Бот был заблокирован пользователем, пользователь {user_id} будет удалён из подписчиков."
        )
        ...
        logger.error(
            f"При отправке уведомления пользователю {user_id} произошла ошибка:",
            exc_info=error,
        )
...
async def _log_subscription_status_failure(response: Response, user_id: int) -> None:
    ...
    error_text = f"При изменении статуса уведомлений пользователя с id={user_id} вернулся статус: {response.status_code}\n"
...
async def _send_event_messages(
    ...
    for event in events:
        message = await _fill_in_message_template_with_event(
            event_data=event, message_template=message_template
        )
        try:
...
async def _fill_in_message_template_with_event(
    event_data: Dict, message_template: str
) -> str:
    ...
    message = message_template.format(
        **event_data, event_time_formatted=event_time_formatted
    )
    return message
